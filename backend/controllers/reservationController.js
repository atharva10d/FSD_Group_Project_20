...
const createReservation = async (req, res) => {
    try {
        ...
        // Validate booking logic based on user role
        if (req.user.role === 'passenger' && passengers.length > 1) {
            return res.status(403).json({ message: 'Passengers can only book 1 seat per reservation. Agents can book multiple.' });
        }
